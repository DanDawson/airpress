...
function airpress_ep_menu() {

	add_submenu_page(
		"airpress_settings", // parent slug
		"Extended Posts", // page title
		"Extended Posts", // menu title
		"manage_options", // capability
		"airpress_ep", // menu_slug
		"airpress_ep_render" // function
	);

}
add_action( 'admin_menu', 'airpress_ep_menu' );

add_action( 'init', "airpress_ep_add_rules" );

// airpress_debug(0,"These are all teh hooks",$wp_filter);

//add_action('rewrite_rules_array','airpress_ep_update_permalinks');
//permalink_structure_changed
//generate_rewrite_rules

function airpress_ep_render( $active_tab = '' ) {
	global $airpress;
...
<div class="wrap">
  ...
  <?php
		$configs = get_airpress_configs("airpress_ep",false);
  ...
  <h2 class="nav-tab-wrapper">
    <?php
			...
			?>
    <a href="?page=airpress_ep&tab=<?php echo $key; ?>"
      class="nav-tab <?php echo $class; ?>"><?php echo $config["name"]; ?></a>
    <?php
			endforeach;
			?>
    <a href="?page=airpress_ep&tab=<?php echo count($configs);?>" class="nav-tab">+</a>
  </h2>

  <form method="post" action="options.php">
    <?php
				settings_fields( 'airpress_ep'.$active_tab );
				do_settings_sections( 'airpress_ep'.$active_tab );		
				submit_button();
...
// 		( isset($_GET["page"]) && strpos($_GET["page"],"airpress_ep") === false ) || 
// 		( isset($_POST["option_page"]) && strpos($_POST["option_page"],"airpress_ep") === false )
// 	){
...
// 		delete_airpress_config("airpress_ep",$_GET['tab']);
// 		header("Location: ".admin_url("/admin.php?page=airpress_ep"));
// 		exit;
// 	} else {
// 		$configs = get_airpress_configs("airpress_ep",false);
...
// 		set_airpress_config("airpress_ep",$active_tab,$config);		
// 	} else {
...
function airpress_admin_vp_tab($key,$config) {
	...
	register_setting($option_name,$option_name,"airpress_ep_validation");
}

function airpress_ep_validation($config){
	// global $wp_rewrite;
	// airpress_ep_add_rule($config);
	// $wp_rewrite->flush_rules();
...
function airpress_ep_add_rule($config){
	return add_rewrite_rule($config["pattern"], 'index.php?page_id='.$config["template"] , 'top');
}

function airpress_ep_add_rules(){
	global $wp_rewrite;

	$configs = get_airpress_configs("airpress_ep");
...
function airpress_admin_vp_render_element_delete($args) {
	...
	$tab = intval($_GET["tab"]);
	echo "<a href='?page=airpress_ep&tab=$tab&delete=true'>Yes, delete this configuration</a>";
}
